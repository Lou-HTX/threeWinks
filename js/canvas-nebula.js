...
$(document).ready(function(){	
	...
	(function ($) {			
			...
			var	Puff = function(p) {				
				var	opacity,
					sy = (Math.random()*850)>>0,
					sx = (Math.random()*550)>>0;
				...
				this.move = function(timeFac) {						
					...
					ctx.drawImage($canvas3[0], sy+p, sy+p, 850-(p*2),550-(p*2), 0,0, w, h);								
				};
			};
			...
			{								
				...
				ctx2.drawImage( $canvas[0] ,0,0,1700,1100);				
				setTimeout(loop, 250 );				
			};
			...
			$(img).bind('load',null, function() {  ctx3.drawImage(img, 0,0, 1700, 1100);	loop(); });
